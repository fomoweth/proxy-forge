...
contract ProxyForgeTestFuzzTest is BaseTest {
    ...
    function test_fuzz_deployAndCall(address caller, address owner)
        public
        assumeEOAs(caller, owner)
        impersonate(caller)
    {
        address proxy = forge.computeProxyAddress(vm.getNonce(address(forge)));
        address admin = forge.adminOf(proxy);
    ...
    function test_fuzz_deployAndCall(address caller, address owner, uint256 initValue, uint256 value)
        public
        assumeEOAs(caller, owner)
        impersonate(caller)
    {
    ...
    function test_fuzz_deployDeterministic(bool protected, address caller, address owner, uint96 identifier)
        public
        assumeEOAs(caller, owner)
        impersonate(caller)
    {
    ...
    function test_fuzz_deployDeterministic_revertsWithInvalidSalt(address caller, address other, uint96 identifier)
        public
        assumeEOAs(caller, other)
        impersonate(caller)
    {
    ...
    function test_fuzz_deployDeterministicAndCall(bool protected, address caller, address owner, uint96 identifier)
        public
        assumeEOAs(caller, owner)
        impersonate(caller)
    {
    ...
    function test_fuzz_deployDeterministicAndCall(
        ...
        try forge.deployDeterministicAndCall{value: value}(implementationV1, owner, salt, data) returns (address proxy)
        {
            assertEq(proxy, predicted);
            assertEq(proxy.balance, value);
            verifyProxyStates(proxy, admin, implementationV1, owner);
    ...
    function test_fuzz_upgrade_revertWithUnauthorizedAccount(address owner, address invalidOwner)
        public
        assumeEOAs(owner, invalidOwner)
    {
    ...
    function test_fuzz_upgradeAndCall(address caller, string memory data, uint256 value)
        public
        assumeEOA(caller)
        impersonate(caller)
    {
